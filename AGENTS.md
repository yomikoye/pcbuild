...
7. Track A RAM procedure:
   - Aggressively search factory-matched 4x64GB 256GB ECC RDIMM kits from G.Skill T5 Neo, Kingston workstation families, Micron, Samsung and reputable EU workstation-memory suppliers.
   - Do not treat four individually expensive Micron modules as the only valid route or recommend 28,000+ PLN RAM merely because it is listed.
   - If 256GB is unavailable or irrationally expensive, also source and calculate a proper matched 128GB 4x32GB ECC RDIMM fallback.
...
9. Track B RAM procedure: use a factory-matched 128GB 2x64GB DDR5 UDIMM kit as primary. Prefer exact ASUS QVL or memory-vendor validation at DDR5-5600 or stable DDR5-6000 EXPO. Stability outranks memory overclocking. Track 256GB 4x64GB only as a capacity experiment; note AMD's official DDR5-3600 support with four DIMMs and the additional controller load.
10. Track B lane procedure: with Ryzen 9000, the two CPU-fed GPU slots on ProArt X870E-CREATOR operate PCIe 5.0 x8/x8, not x16/x16. Leave `M.2_2` empty because it shares lanes with the second GPU slot and can reduce it to x4. Put the 9100 PRO in `M.2_1` and the second Gen4 SSD in chipset-connected `M.2_4` (or verified equivalent chipset slot).
11. Track B PSU procedure: calculate B1 with PRIME PX-2200 for dual-GPU readiness and B2 with a separate high-quality one-GPU-only 1200W–1600W PSU. Never mix B1 and B2. Verify exact ATX revision and native GPU cable before recommendation.
12. Audit CPU/BIOS, exact memory QVL, case/motherboard/AIO/PSU/GPU dimensions, radiator-plus-fan thickness, slot spacing/lane allocation, M.2 placement, native 12V-2x6 leads, safe bend radius, EU mains requirements, fan groups, GPU support and dual-GPU airflow before any BUY NOW recommendation.
13. Calculate only evidence-backed totals. If an exact required component is unavailable, mark the total provisional/incomplete and identify the reference price used; never hide a non-matching revision inside a compatible total.
14. Every weekly report must contain, in this order:
   ...
   - a compact `Metric | Threadripper | AM5 | Difference` comparison covering CPU, motherboard, RAM, case, cooler, case fans, storage, PSU, RTX 5090, one-GPU total and dual-GPU-ready total;
   - platform-only costs for CPU + motherboard + RAM + cooler + case, excluding GPU/storage/PSU;
   - Track A 256GB current obtainable total, Track A 128GB fallback total and Track B 128GB total;
...
15. The architecture comparison must always state:
   ...
   - AM5 advantages: lower platform price/power, better gaming-focused CPU, cheaper cooling/motherboard/RAM/case, and substantial 128GB capacity.
   - AM5 compromises: dual-channel memory, 128GB preferred configuration, x8/x8 dual GPU, fewer PCIe lanes, M.2/GPU sharing and weaker 256GB+ expansion.
16. Maintain first observed, previous, minimum, maximum, weekly change, total change and 30/90-day context independently for each track. Detect fake discounts against repository history.
17. Track a comparable full prebuilt with whole-system warranty. Require exact CPU, motherboard, RAM, GPU, PSU, cooling, storage, VAT, delivery and cross-border collection/RMA details.
18. Update both README price matrices. Every displayed component price must be a Markdown link directly to the retailer product page so the numeric price is clickable on GitHub. Link unavailable exact targets to manufacturer/configurator evidence and totals to the corresponding report. Do not leave a bare `—`; state `No exact stock` and the exact validated target or non-matching reference.
19. Validate JSON/CSV syntax and all arithmetic, run `git diff --check`, then commit completed report/data/README/config changes to `main` and push to `origin/main`. Never push a partial or failed report.
20. After a successful GitHub push, notify the originating Telegram thread only for a meaningful change: component price/delivered-cost change, historical low, stock change, new qualifying seller, warranty/RMA change, compatibility finding, relevant launch, prebuilt comparison change or recommendation-class change. Include headline figures, what changed, alerts, report path and commit. If nothing meaningful changed, begin the final response exactly `[SILENT]`.
